...
-- 应付余额表（按发票）
SELECT A.F_SERVICETYPE, IF(A.F_BILLTYPE='0', '采购发票', '付款单') AS 单据类型,
A.F_BILLID, A.F_DATE, A.F_WORKNO, A.F_ARPAMT, A.F_PAMT, (@BAL:=@BAL+ A.F_ARPAMT-A.F_RPAMT) AS 余额
FROM 
(
...
-- 参数
showFields=发货代码,单据类型,单号,业务日期,合同号,工作号,备注,应付金额,付款金额,余额
sumFields=应付金额,付款金额
flex=true
disableSort=true
columns=
  P_SERVICETYPE=发货代码,PUR_ORDERHEAD.F_SERVICETYPE,
  P_SUPPLIERID=供应商,PUR_ORDERHEAD.F_SUPPLIERID,
  P_STARTDATE=业务日期起,INV_CHANGEHEAD.F_DATE,
  P_ENDDATE=业务日期止,INV_CHANGEHEAD.F_DATE,
  发货代码=发货代码,,70
  单据类型=单据类型,,100
  单号=单号,,160
  业务日期=业务日期,,100
