...
WHERE A.F_WORKNO = 'UR1912NS186'
GROUP BY B.F_FROMORGNAME, B.F_CONTRACTNO, C.F_GOODSCODE, B.F_PRICE, B.F_SERVICETYPE 
-- 

SELECT B.F_TRANSWHSNAMES AS 供应商, 
        C.F_GOODSCODE AS 物料代码, C.F_GOODSNAME AS 物品名称, C.F_GOODSMODEL AS 规格, C.F_GOODSPACKAGEMODEL AS 包装规格, 
        B.F_REFUNITID AS 常用单位, (F_ACTQTY/C.F_GOODSSTDUNITEXRATE) AS 实发生数量, 
...
GROUP BY B.F_TRANSWHSNAMES, C.F_GOODSCODE, B.F_SERVICETYPE 

--
-- 终版
SELECT B.F_FROMORGNAME AS 供应商, B.F_CONTRACTNO AS 合同号, C.F_GOODSCODE AS 物料代码, C.F_GOODSNAME AS 物品名称, 
        C.F_GOODSMODEL AS 规格, C.F_GOODSPACKAGEMODEL AS 包装规格, B.F_REFUNIT AS 常用单位, B.F_ACTQTY AS 实发生数量, 
        B.F_PRICE AS 单价, 
        B.F_SERVICETYPE AS 发货代码, C.F_GOODSSTDUNITEXRATE AS 库存单位转换率
FROM INV_CHANGEHEAD A 
LEFT JOIN INV_CHANGEBODY B ON A.F_BILLID = B.F_BILLID 
LEFT JOIN BAS_GOODS C ON C.F_GOODSID = B.F_GOODSID 
LEFT JOIN MPS_SOBODY D ON D.F_BILLID = B.F_BILLID 
WHERE A.F_WORKNO = 'KOT1912QD001'
GROUP BY  D.F_TRANSWHSNAMES,B.F_CONTRACTNO, C.F_GOODSCODE, B.F_PRICE, D.F_SERVICETYPE
