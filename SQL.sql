...
SELECT A.F_SERVICETYPE AS 发货代码, IF(A.F_BILLTYPE='0', '采购发票', '付款单') AS 单据类型,
        A.F_BILLID AS 单号, A.F_DATE AS 日期, A.F_WORKNO AD 工作号, A.F_ARPAMT AS 应付金额, 
        A.F_PAMT AS 付款金额, (@BAL:=@BAL+ A.F_ARPAMT-A.F_RPAMT) AS 余额
FROM 
(
SELECT F_PURCHASERNAME AS F_CUSTOMER,F_SERVICETYPE,'0'AS F_BILLTYPE,
        F_INVID AS F_BILLID,F_DATE,F_WORKNO, F_AMT AS F_ARPAMT,0 AS F_RPAMT
        FROM ACC_INVHEAD
        WHERE F_BILLTYPE='2'
        AND F_STATUS>='5' AND F_STATUS<'9'
        AND A.F_DATE>=:START_DATE AND A.F_DATE <= DATE_ADD(:END_DATE, INTERVAL 1 DAY)
        AND (:F_CUSTOMER=' ' OR F_PURCHASERNAME LIKE CONCAT('%',:F_CUSTOMER,'%'))
        AND (F_SERVICETYPE=:P_SERVICETYPE)
         
UNION ALL
SELECT F_DEALINGSNAME AS F_CUSTOMER,F_SERVICETYPE, 'FK' AS F_BILLTYPE,
        F_RPID AS F_BILLID,F_DATE,F_WORKNO, 0 AS F_ARPAMT,IFNULL(F_AMT,0) AS F_RPAMT
        FROM ACC_RPBILLHEAD 
        WHERE F_BILLTYPE='2'
        AND F_RPTYPE='2'
        AND F_PAYTYPE='1'
...
columns=
  ...
  F_CUSTOMER=供应商,PUR_ORDERHEAD.F_SUPPLIERID,
  发货代码=发货代码,,70
  单据类型=单据类型,,100
  单号=单号,,160
  日期=业务日期,,100
