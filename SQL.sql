...
-- 入库明细表
-- V1.0
BAS_GOODS, INV_CHANGEHEAD, INV_CHANGEBODY, MPS_SOBODY, MPS_SOHEAD

SELECT B.F_FROMORGNAME AS 供应商, B.F_CONTRACTNO AS 合同号, 
        C.F_GOODSCODE AS 物料代码, C.F_GOODSNAME AS 物品名称, C.F_GOODSMODEL AS 规格, C.F_GOODSPACKAGEMODEL AS 包装规格, 
        B.F_REFUNIT AS 常用单位, (B.F_ACTQTY/C.F_GOODSSTDUNITEXRATE) AS 实发生数量, 
        (B.F_PRICE*C.F_GOODSSTDUNITEXRATE) AS 单价, B.F_AMT AS 金额, B.F_SERVICETYPE AS 发货代码
FROM INV_CHANGEHEAD A
LEFT JOIN INV_CHANGEBODY B ON A.F_BILLID = B.F_BILLID
LEFT JOIN BAS_GOODS C ON B.F_GOODSID = C.F_GOODSID 
WHERE A.F_WORKNO = 'KOT1912QD001'
GROUP BY B.F_FROMORGNAME, B.F_CONTRACTNO, C.F_GOODSCODE, B.F_PRICE, B.F_SERVICETYPE 
-- 
-- 正确 
SELECT B.F_TRANSWHSNAMES AS 供应商, E.F_CONTRACTNO AS 合同号, 
        C.F_GOODSCODE AS 物料代码, C.F_GOODSNAME AS 物品名称, C.F_GOODSMODEL AS 规格, C.F_GOODSPACKAGEMODEL AS 包装规格, 
        B.F_REFUNITID AS 常用单位, (B.F_ACTQTY/C.F_GOODSSTDUNITEXRATE) AS 实发生数量, E.F_PRICE AS 单价, 
        B.F_SERVICETYPE AS 发货代码, 
        C.F_GOODSSTDUNITEXRATE AS 库存单位转换率 
FROM MPS_SOHEAD A
LEFT JOIN MPS_SOBODY B ON A.F_BILLID = B.F_BILLID
LEFT JOIN BAS_GOODS C ON B.F_GOODSID = C.F_GOODSID

LEFT JOIN INV_CHANGEHEAD D ON B.F_WORKNO = D.F_WORKNO
LEFT JOIN INV_CHANGEBODY E ON D.F_BILLID = E.F_BILLID

WHERE B.F_WORKNO = 'KOT1912QD001'
GROUP BY B.F_TRANSWHSNAMES, E.F_CONTRACTNO, C.F_GOODSCODE, E.F_PRICE, B.F_SERVICETYPE 

--
-- 终版
SELECT B.F_FROMORGNAME AS 供应商, B.F_CONTRACTNO AS 合同号, C.F_GOODSCODE AS 物料代码, C.F_GOODSNAME AS 物品名称, 
        C.F_GOODSMODEL AS 规格, C.F_GOODSPACKAGEMODEL AS 包装规格, B.F_REFUNIT AS 常用单位, 
        (B.F_ACTQTY/C.F_GOODSSTDUNITEXRATE) AS 实发生数量, 
        (B.F_PRICE*C.F_GOODSSTDUNITEXRATE) AS 单价, B.F_SERVICETYPE AS 发货代码, C.F_GOODSSTDUNITEXRATE AS 库存单位转换率
        
FROM INV_CHANGEHEAD A 
LEFT JOIN INV_CHANGEBODY B ON A.F_BILLID = B.F_BILLID 
LEFT JOIN BAS_GOODS C ON C.F_GOODSID = B.F_GOODSID  
WHERE A.F_WORKNO = 'KOT1912QD001' 
AND B.F_BILLTYPE = '0' AND B.F_STATUS>=5
GROUP BY  B.F_FROMORGNAME,B.F_CONTRACTNO, C.F_GOODSCODE, B.F_PRICE, B.F_SERVICETYPE 

-- 无供应商、合同号、金额
SELECT B.F_TRANSWHSNAMES AS 供应商, D.F_CONTRACTNO AS 合同号, 
        C.F_GOODSCODE AS 物料代码, C.F_GOODSNAME AS 物品名称, C.F_GOODSMODEL AS 规格, C.F_GOODSPACKAGEMODEL AS 包装规格, 
        B.F_REFUNIT AS 常用单位, (B.F_ACTQTY/C.F_GOODSSTDUNITEXRATE) AS 实发生数量, 
        B.F_SERVICETYPE AS 发货代码, D.F_PRICE AS 单价, 
        C.F_GOODSSTDUNITEXRATE AS 库存单位转换率
FROM MPS_SOHEAD A 
LEFT JOIN MPS_SOBODY B ON A.F_BILLID = B.F_BILLID 
LEFT JOIN BAS_GOODS C ON B.F_GOODSID = C.F_GOODSID 
LEFT JOIN INV_CHANGEBODY D ON D.F_BILLID = B.F_BILLID
WHERE B.F_WORKNO = 'KOT1912QD001'
GROUP BY B.F_TRANSWHSNAMES, D.F_CONTRACTNO, C.F_GOODSCODE, D.F_PRICE, B.F_SERVICETYPE 
...
SELECT A.F_STATUS AS 状态, B.F_SUPPLIER AS 供应商, C.F_CONTRACTNO AS 合同号, E.F_GOODSCODE AS 物料代码, E.F_GOODSNAME AS 物品名称, E.F_GOODSMODEL AS 规格, 
E.F_GOODSPACKAGEMODEL AS 包装规格, D.F_REFUNIT AS 常用单位, round(D.F_PRICE*D.F_EXRATE, 4) AS 采购单价, (B.F_ACTQTY/D.F_EXRATE) AS 实发生数量, SUM((B.F_ACTQTY/D.F_EXRATE)) AS 总数量, 
round(((D.F_PRICE*D.F_EXRATE)*(B.F_ACTQTY/D.F_EXRATE)), 4) AS 金额, 
SUM(((D.F_PRICE*D.F_EXRATE)*(B.F_ACTQTY/D.F_EXRATE))) AS AMT, 
...
WHEN B.F_CI='8000' THEN '加纳'
WHEN B.F_CI='9000' THEN '尼日利亚' END AS 国家,
B.F_ACCOUNTNAME, A.F_DATE, A.F_ORGNAME, B.F_BANKACCOUNT, A.F_CURRENCY, A.F_AMT, A.F_INAMT, A.F_ACCEPTAMT, A.F_OUTAMT, 
A.F_NETAMT, A.F_EXRATE, A.F_NETAMT_USD, (A.F_NETAMT_USD/10000) AS 万美元, A.F_REMARK 
FROM FC_ACCLOG A 
LEFT JOIN DB_ORGS B ON A.F_ORGNAME = B.F_NAME 
WHERE A.F_STATUS = 1
...
WHEN B.F_ROOTID='3000' THEN '南非'
WHEN B.F_ROOTID='4000' THEN '肯尼亚'
WHEN B.F_ROOTID='5000' THEN '莫桑楠普拉'
WHEN B.F_ROOTID='6000' THEN '中国香港'
WHEN B.F_ROOTID='7000' THEN '莫桑马普托'
WHEN B.F_ROOTID='8000' THEN '加纳'
WHEN B.F_ROOTID='9000' THEN '尼日利亚' END AS 国家,
B.F_ACCOUNTNAME, A.F_DATE, A.F_ORGNAME, B.F_BANKACCOUNT, A.F_CURRENCY, A.F_AMT, A.F_INAMT, A.F_ACCEPTAMT, A.F_OUTAMT, 
A.F_NETAMT, A.F_EXRATE, A.F_NETAMT_USD, (A.F_NETAMT_USD/10000) AS 万美元, A.F_REMARK 
FROM DB_ORGS B 
